...
// import { ArgumentError, DatabaseError } from '../utils/errors'
...
// userSchema.static('updatePassword', async function (userID: string, newPassword: string) {
...
// 	return User.findOne({ username: auth.username }, 'id login').then((user) => {
// 		if (!user) throw new LoginError('Unknown username')

// 		if (user.login.password !== encrypt(auth.password!, user.login.salt)) throw new LoginError('Incorrect password')

// 		return user
// 	})
// })

// userSchema.static('checkPassword', async function (userID: string, password: string): Promise<boolean> {
// 	// TODO: Determine what I actually need from the login
// 	let user = await User.findById(userID, 'id login')

// 	if (!user) throw new LoginError(`Unknown userID ${userID}`)

// 	if (user.login.password !== encrypt(password, user.login.salt)) throw new LoginError('Incorrect password')

// 	return true
// })

// userSchema.static('addEntry', async function (userID: string, entryID: string): Promise<any> {
...
// userSchema.static('removeEntry', async function (userID: string, entryID: string): Promise<boolean> {
...
// 		if (index < 0) throw new ArgumentError(`Unknown entryID ${entryID} for userID ${userID}`)
...
// userSchema.static('updateUserConfig', async function (userID: string, newConfig: IUserConfig) {
...
//  */
// export const User: IUserModel = model<IUserDoc, IUserModel>(CollectionName.name, userSchema, CollectionName.collection)
